...
- (int)getFilterMode:(const char*)spotId {
    return [ImobileSdkAds getFilterModeBySpotId:[NSString stringWithUTF8String:spotId]];
}
...
- (void)imobileSdkAdsSpotDidGetFilterMode:(NSString *)spotId {
    ...
    for (NSString *gameObjectName in gameObjectNames ) {
        UnitySendMessage([gameObjectName UTF8String],
                         [@"imobileSdkAdsSpotDidGetFilterMode" UTF8String],
                         [msg UTF8String]);
    }
}
...
extern "C" {
    ...
    int getFilterMode_(const char* spotId) {
        if (!unityPlugin) {
            unityPlugin = [[ImobileSdkAdsIosUnityPluginImpl alloc] init];
        }
        return [unityPlugin getFilterMode:spotId];
    }
